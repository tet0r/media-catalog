...
const SORT_COLUMNS = new Set(['title', 'artist', 'year', 'added_at', 'date_added']);

router.get('/', (req, res) => {
  const { q, sort = 'title', dir = 'asc' } = req.query;
  let sql = 'SELECT * FROM vinyl_records WHERE 1=1';
  const params = [];
  if (q) {
...
router.post('/sync', (req, res) => {
  const status = db.prepare('SELECT * FROM vinyl_sync_status WHERE id = 1').get();
...
router.get('/sync/status', (req, res) => {
  res.json(db.prepare('SELECT * FROM vinyl_sync_status WHERE id = 1').get());
});

// Wipes the local mirror — used by Settings' "Clear Library". Items come
// back on the next sync (this app never deletes anything from the actual
// Discogs collection), so this is really "start the local copy over"
// rather than a true collection wipe the way it is for the other media
// types.
router.post('/clear-all', (req, res) => {
  const rows = db.prepare('SELECT cover_file FROM vinyl_records').all();
  for (const row of rows) {
    if (!row.cover_file) continue;
    try { fs.unlinkSync(path.join(DATA_DIR, 'posters', row.cover_file)); } catch { /* already gone, fine */ }
  }
  const info = db.prepare('DELETE FROM vinyl_records').run();
  res.json({ ok: true, count: info.changes });
});

router.get('/:id', (req, res) => {
  const row = db.prepare('SELECT * FROM vinyl_records WHERE id = ?').get(req.params.id);
...
router.put('/:id/cover', async (req, res) => {
  try {
    ...
    db.prepare('UPDATE vinyl_records SET cover_file = ? WHERE id = ?').run(filename, req.params.id);
    const row = db.prepare('SELECT * FROM vinyl_records WHERE id = ?').get(req.params.id);
    if (!row) return res.status(404).json({ error: 'Not found' });
    res.json(rowToRecord(row));
  } catch (err) {
...
// also removed from the actual Discogs collection, since Discogs (not this
// app) is the source of truth for what's in the collection.
router.delete('/:id', (req, res) => {
  db.prepare('DELETE FROM vinyl_records WHERE id = ?').run(req.params.id);
  res.status(204).end();
});
