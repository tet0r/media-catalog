...
const SORT_COLUMNS = new Set(['title', 'artist', 'year', 'added_at']);
...
// album_scan_pending/album_ignored/album_scan_status: those are about
// specific folders on disk, not the collection's contents.
router.post('/clear-all', (req, res) => {
  const rows = db.prepare('SELECT cover_file FROM albums').all();
  for (const row of rows) {
    if (!row.cover_file) continue;
    try { fs.unlinkSync(path.join(DATA_DIR, 'posters', row.cover_file)); } catch { /* already gone, fine */ }
  }
  const info = db.prepare('DELETE FROM albums').run();
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`UPDATE albums SET ${updates.join(', ')} WHERE id = @id`).run(params);
  const row = db.prepare('SELECT * FROM albums WHERE id = ?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'Not found' });
  res.json(rowToAlbum(row));
});
...
router.put('/:id/cover', async (req, res) => {
  try {
    ...
    db.prepare('UPDATE albums SET cover_file = ? WHERE id = ?').run(filename, req.params.id);
    const row = db.prepare('SELECT * FROM albums WHERE id = ?').get(req.params.id);
    if (!row) return res.status(404).json({ error: 'Not found' });
    res.json(rowToAlbum(row));
  } catch (err) {
